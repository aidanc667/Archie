...
**The type-level guarantee above is necessary but not sufficient — the free-text `report` field is a separate leak channel.** The top 3 files by risk score get their *full raw source* embedded verbatim into the Context Pack (`ContextPack.topRiskFiles[i].source`, `src/summarizer.ts`) so the model can write specific, evidenced findings about them. If a `security.secrets` hit falls on one of those files, that file's real secret text would otherwise sit directly in the LLM prompt (`JSON.stringify(pack, ...)`, `src/reasoning.ts`) — and no prompt instruction reliably stops a model from quoting text already in its own context back out as "Evidence" in a risk write-up, which then gets posted as a public/org-visible GitHub comment. `SecurityFinding` never carrying the value doesn't protect against this, because the model isn't reading `SecurityFinding` for the secret — it's reading the file's own source. `buildContextPack` (`src/summarizer.ts`) closes this by redacting every line `security.secrets` flags out of a file's embedded source *before* it enters `ContextPack` at all, independent of any grounding-rule wording — the value is gone from the model's context by the time the prompt is built, not merely discouraged from being repeated.
